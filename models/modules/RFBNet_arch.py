...
class ReceptiveFieldDenseBlock(nn.Module):
    ...
    def __init__(self, nf= 64, gc= 32, scale_ratio: float = 0.1):
        super(ReceptiveFieldDenseBlock, self).__init__()
        self.rfb1 = nn.Sequential(
            ReceptiveFieldBlock(nf + 0 * gc, gc),
            nn.LeakyReLU(negative_slope=0.2, inplace=True)
        )

        self.rfb2 = nn.Sequential(
            ReceptiveFieldBlock(nf + 1 * gc, gc),
            nn.LeakyReLU(negative_slope=0.2, inplace=True)
        )

        self.rfb3 = nn.Sequential(
            ReceptiveFieldBlock(nf + 2 * gc, gc),
            nn.LeakyReLU(negative_slope=0.2, inplace=True)
        )

        self.rfb4 = nn.Sequential(
            ReceptiveFieldBlock(nf + 3 * gc, gc),
            nn.LeakyReLU(negative_slope=0.2, inplace=True)
        )

        self.rfb5 = ReceptiveFieldBlock(nf + 4 * gc, nf)
...
class ResidualOfReceptiveFieldDenseBlock(nn.Module):
    ...
    def __init__(self, nf= 64, gc= 32, scale_ratio: float = 0.1):
        ...
        self.RFDB1 = ReceptiveFieldDenseBlock(nf, gc)
        self.RFDB2 = ReceptiveFieldDenseBlock(nf, gc)
        self.RFDB3 = ReceptiveFieldDenseBlock(nf, gc)
...
class RFBNet(nn.Module):
    def __init__(self) -> None:
        ...
        self.RFB = ReceptiveFieldBlock(64, 64, 0.2)
...
#        trunk_rfb = self.Trunk_RFB(conv1)
        out = torch.add(conv1, trunk_rfb)
